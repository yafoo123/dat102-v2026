...
public class LenketBagRekursiv<T> implements BagADT<T> {
	...
	private Node finnNode(T entry, Node head) {
		
		if (head == null) { // basistilfelle 1
			return head;
			
		} else if(head.data.equals(entry)) { // basistilfelle 2
			return head;
			
		} else {
			return finnNode(entry, head.neste); // vi leter videre i resten ...
		}
	}
     ...
     */
    private Node finnNodeKompakt(T entry, Node head) {
        return (head == null || head.data.equals(entry))
                ? head
                : finnNodeKompakt(entry, head.neste);
    }
     ...
     * Rekursiv løsning
     */
    private int getFrequencyOf(T anEntry, Node head) {
        if (head == null) { //basistilfelle
            return 0;
        }
...
//		int antallIDenne = head.data.equals(anEntry) ? 1 : 0;

        return antallIDenne + getFrequencyOf(anEntry, head.neste);
    }
     ...
     * Litt mer kompakt kode for rekursiv løsning
     */
    private void skrivUtKompakt(Node head) {
        if (head != null) {
            System.out.println(head.data);
            skrivUtKompakt(head.neste);
        }
    }
