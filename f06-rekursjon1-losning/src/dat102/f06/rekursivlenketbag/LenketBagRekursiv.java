...
public class LenketBagRekursiv<T> implements BagADT<T> {
	 ...
	 */
	private Node finnNode(T entry) {
		return finnNode(forste, entry);
	}
	
	/*
	 * Rekursiv løsning
	 */
	private Node finnNode(Node start, T entry) {
		if (start == null) { //Basistilfelle 1
			return null; // evt. return start, som er null.
		}
		if (start.data.equals(entry)) { //Basistilfelle 2
			return start;
		}
		return finnNode(start.neste, entry);
	}
	
	/*
	 * Litt mer kompakt kode for rekursiv løsning
	 */
	private Node finnNodeKompakt(Node start, T entry) {
		return (start == null || start.data.equals(entry))
				? start
				: finnNode(start.neste, entry);
	}
	...
	@Override
	public int getFrequencyOf(T anEntry) {
		return getFrequencyOf(forste, anEntry);
	}
	
	/*
	 * Rekursiv løsning
	 */
	private int getFrequencyOf(Node start, T anEntry) {
		if (start == null) { //basistilfelle
			return 0;
		}
		int telleDenne = 0;
		if (start.data.equals(anEntry)) {
			telleDenne = 1;
		}
//		evt.		
//		int telleDenne = start.data.equals(anEntry) ? 1 : 0;
		return telleDenne + getFrequencyOf(start.neste, anEntry);
	}
	 ...
	 * Rekursiv løsning
	 */
	private void skrivUt(Node start) {
		if (start == null) { //Basistilfelle.
			//Ingenting å gjøre. Kunne snudd test og "sløyfet" basistilfelle.
		}
	 ...
	 * Litt mer kompakt kode for rekursiv løsning
	 */
	private void skrivUtKompakt(Node start) {
		if (start != null) {
			System.out.println(start.data);
			skrivUt(start.neste);
		}
